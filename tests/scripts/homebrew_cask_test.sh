...
tap_name="pk-release/cask-smoke"
cask_name="pk-release-smoke"
cask_ref="$tap_name/$cask_name"
binary_target="pk-release-smoke"
...
sh "$repo_root/scripts/verify-homebrew-cask.sh" "$dist_dir"
...
set -- "$dist_dir"/pk_darwin_"$goarch"_*/pk
[ "$#" -eq 1 ] || fail "expected one darwin/$goarch snapshot artifact"
[ -f "$1" ] || fail "missing darwin/$goarch snapshot artifact"
snapshot_binary="$1"
asset_name="pk-darwin-$goarch"

test_root="$(mktemp -d "${TMPDIR:-/tmp}/pk-cask-smoke.XXXXXX")"
...
sed \
  -e "s/^cask \"pk\" do$/cask \"$cask_name\" do/" \
  -e "s|url \"[^\"]*/$asset_name\"|url \"file://$local_artifact\"|" \
  -e "s/target: \"pk\"/target: \"$binary_target\"/g" \
...
grep -Fq "url \"file://$local_artifact\"" "$test_cask" || fail "local URL was not substituted"
delete_probe='args: ["-d", "com.apple.quarantine", binary]'
[ "$(grep -Fc "$delete_probe" "$test_cask")" -eq 2 ] || fail "missing quarantine precondition"
ruby -c "$test_cask" >/dev/null

git -C "$tap_repo" init --quiet
git -C "$tap_repo" add "Casks/$cask_name.rb"
git -C "$tap_repo" -c user.name=Homebrew -c user.email=brew@localhost \
...
[ "$actual_version" = "pk v$expected_version" ] || fail "installed binary version mismatch"
...
if /usr/bin/xattr -p com.apple.quarantine "$installed_artifact" >/dev/null 2>&1; then
  fail "installed artifact still has quarantine"
fi
